...
The **electron-volt** ($\mbox{eV}$) is the amount of energy needed to move an electron through a potential difference of one volt.
...
Let the charge on each electron be $q$, and assume an electron density in the metal of $n$electrons per unit volume. Additionally, let us assume that all of the electrons have an average drift speed $v$. In a time interval $\Delta t$ all electrons within the cylindrical volume of the area $V$ and length $v\Delta t$ will have passed out of the end face of the cylinder. The total charge that left the cylinder is $I = nAvq$, (the number of electrons multiplied by the charge on the electron).
